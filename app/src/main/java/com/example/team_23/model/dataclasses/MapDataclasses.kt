...
data class Routes(val bounds: Bounds?, val copyrights: String?, val legs: List<Legs>?, val overview_polyline: OverviewPolyline?, val summary: String?, val warnings: List<Any>?, val waypoint_order: List<Any>?)
data class Southwest(val lat: Number?, val lng: Number?)
data class StartLocation(val lat: Number?, val lng: Number?)
data class Steps(val distance: Distance?, val duration: Duration?, val end_location: EndLocation?, val html_instructions: String?, val maneuver: String?, val polyline: Polyline?, val start_location: StartLocation?, val travel_mode: String?)